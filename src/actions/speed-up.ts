...
@action({ UUID: "com.jdsoliveiraa.fitdeck.speed-up" })
export class SpeedUpAction extends SingletonAction<SpeedSettings> {
	...
	override async onKeyDown(ev: KeyDownEvent<SpeedSettings>): Promise<void> {
		...
		const newSpeed = Math.min(treadmillService.currentSpeed + step, treadmillService.maxSpeed);
